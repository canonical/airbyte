...
The `expenses` stream sources the same underlying `combinedReportData` export as `reports`, but uses a different template to flatten each report's nested transaction list into one row per expense. Since transaction-level rows don't carry a report's `submitted`/`approved`/`reimbursed` dates, the `updatedAt` cursor is instead derived from the most recent non-null value across the `created`, `modifiedCreated`, and `inserted` date columns, and the internal export-window cursor (`createdAt`) tracks only the `created` column. The same `lookback_window_days` trailing-window strategy described above applies here as well.
